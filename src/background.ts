...
async function unlock(extra?: number, reason?: string) {
  if (extra !== undefined && (!Number.isInteger(extra) || extra < 1 || extra > 30 || typeof reason !== 'string' || reason.trim().length < 80)) throw new Error('Invalid extra time request');
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  ...
  const operation = async () => {
    ...
    return { ok: true, unlockedUntil: await unlock(message.minutes, message.reason) };
  };
